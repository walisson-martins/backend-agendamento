...
public class PatientController {
    ...
    public ResponseEntity<BaseResponseDTO> salvar(@RequestBody PatientDTO patientDTO) {
        Patient patient = patientService.fromDto(patientDTO);

        return ResponseEntity.status(201).body(new BaseResponseDTO("Foi cadastrado com sucesso!", patientService.salvar(patient)));
    }
}
